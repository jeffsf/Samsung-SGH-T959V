...
void dtun_dm_sig_auth_comp(tDTUN_DEVICE_SIGNAL *msg)
{
    ...
    if( !st )
    {
        ...
        dummy_handle++;
    } else if (st == HCI_OE_USER_ENDED_CONNECTION) {
        info("%s: call hcid_dbus_conn_complete to remove device.", __func__);
        hcid_dbus_conn_complete(&sba, st, dummy_handle,  &msg->auth_comp.info.bd_addr );
    }
